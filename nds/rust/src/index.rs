...
pub fn start() {
    ...
    loop {
        ...
        if frame_count >= 6 && !paused {
            ...
            player.body.insert(0, match player.direction {
            ...
            while player.body.len() > player.score as usize {
                player.body.pop();
            }

            for segment in &player.body[1..] {
                if player.body[0] == *segment {
                    player = Player::new();
                    food = new_food();
                    break;
                }
            }
            if player.body[0].0 >= GRID_DIMENSIONS.0 || player.body[0].1 >= GRID_DIMENSIONS.1 {
                player = Player::new();
                food = new_food();
            }
        }
